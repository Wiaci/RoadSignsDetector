...
По умолчанию Docker предоставляет порт 8080, поэтому измените его в Dockerfile при необходимости. Когда все будет готово, просто используйте Dockerfile для построения образа.
...
Это подключит необходимые зависимости и проверьте развертывание, перейдя по адресу вашего сервера в предпочитаемом вами браузере http://localhost:3000.

> Заметка: Убедитесь, что другие копии контейнеров не запущены. Используйте `docker ps` для вывода списка контейнеров и `docker rm -f <ids>` для их удаления..
